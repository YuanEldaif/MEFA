...
class _SdeintAdjointMethod(torch.autograd.Function):
    ...
    def forward(ctx, sde, ts, dt, bm, solver, method, adjoint_method, adjoint_adaptive, adjoint_rtol, adjoint_atol,
        ...
        ys, noises, extra_solver_state = solver.integrate(y0, ts, extra_solver_state)
        ...
        return (ys, noises, *extra_solver_state)
...
def sdeint_adjoint(sde: nn.Module,
    ...
    ys, noises, *extra_solver_state = _SdeintAdjointMethod.apply(
    ...
    return sdeint.parse_return(y0, ys, noises, extra_solver_state, extra, logqp)
