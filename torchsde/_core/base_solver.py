...
class BaseSDESolver(metaclass=better_abc.ABCMeta):
    ...
    @abc.abstractmethod
    def step(self, t0: Scalar, t1: Scalar, y0: Tensor, extra0: Tensors) -> Tuple[Tensor, Tensor, Tensors]:
    ...
    def integrate(self, y0: Tensor, ts: Tensor, extra0: Tensors) -> Tuple[List[Tensor], List[Tensor], List[Tensor], List[Tensor], Tensors]:
        ...
        for out_t in ts[1:]:
            with torch.no_grad():
                while curr_t < out_t:
                    ...
                    curr_ts.append(curr_t.detach().cpu())
                    next_ts.append(next_t.detach().cpu())
                    ...
                    if self.adaptive:
                        ...
                        prev_t, prev_y = curr_t, curr_y
                        curr_y, curr_noise, curr_extra = self.step(curr_t, next_t, curr_y, curr_extra)
                        # print(f'curr_y output: {curr_y}:')
                        curr_t = next_t
                        ys.append(curr_y.detach().cpu())
                        noises.append(curr_noise.detach().cpu())
            # ys.append(interp.linear_interp(t0=prev_t, y0=prev_y, t1=curr_t, y1=curr_y, t=out_t))
            # noises.append(curr_noise)
        # return torch.stack(ys, dim=0), torch.stack(noises, dim=0), torch.stack(curr_ts, dim=0), torch.stack(next_ts, dim=0), curr_extra
        return ys, noises, curr_ts, next_ts, curr_extra
