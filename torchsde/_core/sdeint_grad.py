...
def sdeint_grad(sde,
    ...
    """
    misc.handle_unused_kwargs(unused_kwargs, msg="`sdeint_grad`")
    del unused_kwargs
    ...
    net_grads, extra_solver_state = solver.integrate(ys, ts, noises, curr_ts, next_ts, net_grads, extra_solver_state, device = device)

    return parse_return(ys, net_grads, extra_solver_state, extra, logqp)
...
def check_contract(sde, y0, ts, bm, method, adaptive, options, names, logqp):
    ...
    if bm is None:
        ...
        bm = BrownianInterval(t0=ts[0], t1=ts[-1], size=(batch_sizes[0], noise_sizes[0]), dtype=y0.dtype,
                              device=y0.device, levy_area_approximation=levy_area_approximation, entropy=42)
...
def parse_return(ys, net_grads, extra_solver_state, extra, logqp):
    if logqp:
        ...
        if extra:
            return net_grads, extra_solver_state
        else:
            return net_grads
